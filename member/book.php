...
if($dopost=='')
{
    ...
    $totalRow = $row['dd'];
    include(dirname(__FILE__)."/templets/book@@.htm");
}
...
function GetList(&$dsql, $pageno, $pagesize)
{
    ...
    while($row = $dsql->GetArray())
    {
        ...
        $line .= "<table width='100%' border='0' cellpadding='0' cellspacing='0' class='list'>
          <thead>
            <tr>
              <th colspan='6'><strong class='fLeft' style='padding-left: 5px;'>留言标题：<font color='#000'>".$row['title']."</font></strong><span class='fRight'>
          ...
          <tbody>
            <tr>
              <td width='18%' style='padding-left: 10px;'>用户昵称：".$row['uname']."</td>
              <td>留言时间：".MyDate("Y-m-d H:i",$row['dtime'])."";
        ...
        $line .= "</td>
            ...
            <tr>
              <td colspan='2' style='padding-left: 10px;'>留言内容：".Text2Html($row['msg'])."</td>
            </tr>
          </tbody>
        </table>";
    }
