...
if($step == 1)
{
    ...
    if($dopost=='regbase')
    {
       ...
       '0','$spaceSta','','$safequestion','$safeanswer','$jointime','$joinip','$logintime','$loginip'); ";
        if($dsql->ExecuteNoneQuery($inQuery))
        {
            ...
            //写入其它默认数据
            $dsql->ExecuteNoneQuery("INSERT INTO `#@__member_flink`(mid,title,url) VALUES('$mid','艺尚创意网','http://www.ies5.com'); ");
            ...
            if($cfg_mb_reginfo == 'Y' && $spaceSta >=0)
            {
                ...
                require_once(DEDEMEMBER."/templets/reg_3@@.htm");
                exit;
            } 
        } else {
    ...
    require_once(DEDEMEMBER."/templets/reg_1@@.htm");
} else {
    if(!$cfg_ml->IsLogin())
    {
    ...
    require_once(DEDEMEMBER."/templets/reg_2@@.htm");
}
