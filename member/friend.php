...
if($dopost=='upsta')
{
    ...
    if($sta=='good')
    {
        ShowMsg("成功把指定好友设为关注好友！","friend.php?ftype=1");
    }
    else if($sta=='bad')
    {
        ShowMsg("成功把指定好友放入黑名单！","friend.php?ftype=-1");
    }
    else
    {
        ShowMsg("成功把指定好友转为普通好友！","friend.php");
    }
    exit();
}
...
else if($dopost=='del')
{
    ...
    ShowMsg("成功删除所选的好友！","friend.php?ftype=".$ftype);
    exit();
}
//浏览
else{
    ...
    $dlist->SetTemplate(dirname(__FILE__).'/templets/friend@@.htm');
