...
if($fmdo=='sendMail')
{
    ...
    $mailbody .= "{$url}\r\n\r\n";
    $mailbody .= "Power by http://www.ies5.com 艺尚创意网\r\n";
...
else if($fmdo=='user')
{
    ...
    if($dopost=="checkuser")
    {
        ...
        else
        {
            ...
            if(defined('UC_API') && @include_once DEDEROOT.'/uc_client/client.php')
            {
                ...
                if($ucresult > 0)
                {
                    echo "<font color='#3D882D'>√用户昵称可用</font>";
                }
                elseif($ucresult == -1)
                {
                    echo "<font color='red'>×用户昵称不合法</font>";
                }
                elseif($ucresult == -2)
                {
                    echo "<font color='red'>×包含要允许注册的词语</font>";
                }
                elseif($ucresult == -3)
                {
                    echo "<font color='red'>×用户昵称已经存在</font>";
                }
                exit();
            }
            #/aip}}            
        ...
        if($cktype!=0 || $cfg_mb_wnameone=='N') {
		 ...
		 if($msg =='ok')
        {
            $msg = "<font color='#3D882D'>√用户名可以使用</font>";
        }
        else
        {
    ...
    else  if($dopost=="checkmail")
    {
        ...
        if(defined('UC_API') && @include_once DEDEROOT.'/uc_client/client.php')
        {
            ...
            if($ucresult > 0) {
                echo "<font color='#3D882D'>√邮箱可以使用</font>";
            } elseif($ucresult == -4) {
                echo "<font color='red'>×邮箱格式有误</font>";
            } elseif($ucresult == -5) {
                echo "<font color='red'>×邮箱不允许注册</font>";
            } elseif($ucresult == -6) {
        ...
        {
            $msg = "<font color='#3D882D'>√邮箱可以使用</font>";
        }
        else
        {
            if(!CheckEmail($email))
            {
                $msg = "<font color='red'>×邮箱格式有误</font>";
            }
            else
            {
                ...
                if(!$rs) {
                    ...
                    if($res) {
                       ...
                       `drink`='0', `datingtype`='0', `language`='', `nature`='', `lovemsg`='', `address`='',`uptime`='0';",
                        ...
                        3 => "INSERT INTO `#@__member_flink` SET `mid`='$mid', `title`='艺尚创意网', `url`='http://www.ies5.com';"
                        );                        
